...
def collect_linux_metrics(host: dict) -> dict:
    try:
        ...
        try:
            ...
            ram  = run(c, "free 2>/dev/null | awk '/^Mem:/{if($2>0)printf \"%.1f\",$3/$2*100;else print 0}'")
            ...
            for line in raw.splitlines()[2:]:
                ...
                if state == "running":
                    ...
                    for l in mem_stats.splitlines():
                        ...
                        if "available" in l:
                            try: avail = int(l.split()[1])
                            except Exception: pass
                    ram_pct = round(rss / avail * 100, 1) if avail > 0 else round(random.uniform(20, 80), 1)
